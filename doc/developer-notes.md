...
- [Development guidelines](#development-guidelines)
    - [General MK2X Core](#general-mk2x-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MK2X Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MK2s (tMK2)" on the test network, if you
...
MK2X Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mk2x-project/mk2x/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mk2x
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mk2x --log_level=test_suite
$ valgrind -v --leak-check=full src/mk2xd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mk2x.coverage/index.html`.
```
...
- BitcoinMiner : Generates MK2s (if wallet is enabled).
...
However, in open source software such as MK2X Core, where everyone uses
...
pay attention to for reviewers of MK2X Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MK2X Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MK2X Core so that they can be integrated
...
file descriptors it will cause problems with MK2X's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MK2X-Project/MK2X.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mk2x-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mk2x-cli` and the GUI debug console use this table to determine how to
