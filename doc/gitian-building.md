...
*Setup instructions for a gitian build of MK2X Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MK2X
...
to the MK2X GitHub release page.
...
- [Building MK2X Core](#building-mk2x-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/mk2x-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/mk2x-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/mk2x-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/mk2x-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/mk2x-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/mk2x-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/mk2x-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@mk2x.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/mk2x-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/mk2x-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/mk2x-osx-*-build.assert
```
