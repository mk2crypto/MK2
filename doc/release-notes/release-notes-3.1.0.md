PIVX Core version *3.1.0* is now available from:  <https://github.com/mk2x-project/mk2x/releases>
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/PIVX-Qt (on Mac) or mk2xd/mk2x-qt (on Linux).
...
zMK2 Staking is here! zMK2 staking will be activated on the morning of the 8th of May 2018. With the release of zMK2 staking, there are effectively 2 versions of zMK2, zMK2 minted on the 3.0.6 PIVX wallet or lower, and zMK2 minted on PIVX wallet version or higher. New features in this release will require the use of zMK2 v2, zMK2 minted on this wallet release 3.1.0 or later. If you currently hold zMK2 v1 and wish to take advantage of zMK2 staking and deterministic zMK2, you will need to spend the zMK2 v1 to yourself and remint zMK2 v2.
...
To encourage the use of zMK2 and increase the PIVX zerocoin anonymity set, the PIVX payment schedule has been changed to the following:
...
### Summary
 ...
 The PIVX wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the mk2x.conf file, found in the PIVX data directory.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the PIVX support group, which often times sees users say "I didn't spend that zMK2". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
Several critical security flaws in the zerocoin protocol and PIVX's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zMK2 Control dialog in the QT wallet, a user is able to see zMK2 marked as version 1 or 2.
