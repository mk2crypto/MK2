MK2X Core version *3.1.0* is now available from:  <https://github.com/mk2x-project/mk2x/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/mk2x-project/mk2x/issues>


Mandatory Update
==============

MK2X Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zMK2 Staking is here! zMK2 staking will be activated on the morning of the 8th of May 2018. With the release of zMK2 staking, there are effectively 2 versions of zMK2, zMK2 minted on the 3.0.6 MK2X wallet or lower, and zMK2 minted on MK2X wallet version or higher. New features in this release will require the use of zMK2 v2, zMK2 minted on this wallet release 3.1.0 or later. If you currently hold zMK2 v1 and wish to take advantage of zMK2 staking and deterministic zMK2, you will need to spend the zMK2 v1 to yourself and remint zMK2 v2.
Note: To find your zMK2 version, click the privacy tab, then the zMK2 Control button then expand the arrows next to the desired denomination.


### Deterministic zMK2 Seed Keys

zMK2 is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zMK2 outside of the wallet that the zMK2 had been minted on. zMK2 can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zMK2 minting

zMK2 minting now only requires 1 further mint (down from 2) to mature. zMK2 mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zMK2 Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zMK2 denomination has been spent or not.
...
The visual layout of the MK2X Qt wallet has undergone a near-complete overhaul.
...
### Summary
 ...
 The MK2X wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the mk2x.conf file, found in the MK2X data directory.
- backuppath = <directory / full path>
- zmk2backuppath = <directory / full path>
...
* If zMK2 backup, auto generated name is `wallet-autozmk2backup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
MK2X Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The MK2X client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running mk2xd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the MK2X support group, which often times sees users say "I didn't spend that zMK2". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
-	Enforced v1 zMK2 spends to require a security level of 100
-	Updates to zMK2 spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the MK2X-Qt masternodes tab
-	Updated MK2X-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and MK2X's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zMK2 Control dialog in the QT wallet, a user is able to see zMK2 marked as version 1 or 2.

zPoS (zMK2 staking)
--------------

Once a zMK2 has over 200 confirmations it becomes available to stake. Staking zMK2 will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zMK2. So for example if a 1,000 zMK2 denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zMK2s.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within MK2X's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
MK2X's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which MK2X used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to MK2X using the bug bounty program. The modulus is now correctly imported and MK2X's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zMK2 v2: zPoS dzMK2 ezMK2 external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zMK2 balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zMK2 status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zMk2 Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault mk2x-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zMK2 spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zMK2. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zMK2. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/mk2x-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
