// Copyright (c) 2018-2020 The MK2X developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zmk2/deterministicmint.h>
#include "zmk2tracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzMK2Tracker::CzMK2Tracker(CWallet* parent)
{
    this->wallet = parent;
...
CzMK2Tracker::~CzMK2Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMK2Tracker::Init()
{
...
bool CzMK2Tracker::Archive(CMintMeta& meta)
{
...
bool CzMK2Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMK2Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMK2Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMK2Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMK2Tracker::GetSerialHashes()
{
...
CAmount CzMK2Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMK2Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMK2Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMK2Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMK2Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMK2Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMK2Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMK2Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMK2Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMK2Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzMK2Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMK2Wallet* zMK2Wallet)
{
    bool iszMK2WalletInitialized = (nullptr != zMK2Wallet);
    CMintMeta meta;
    ...
    if (!iszMK2WalletInitialized)
        zMK2Wallet = new CzMK2Wallet(wallet);
    meta.isSeedCorrect = zMK2Wallet->CheckSeed(dMint);
    if (!iszMK2WalletInitialized)
        delete zMK2Wallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMK2Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMK2Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMK2Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMK2Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMK2Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMK2Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzmk2 from DB\n", __func__, listDeterministicDB.size());
    }
