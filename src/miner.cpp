...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zMK2 spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zMK2 spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zMK2 serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("MK2XMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("MK2XMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("MK2XMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("mk2x-miner");
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("MK2XMiner exception");
    } catch (...) {
        LogPrintf("MK2XMiner exception");
    }

    LogPrintf("MK2XMiner exiting\n");
}
