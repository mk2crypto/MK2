...
#include "qt/mk2x/forms/ui_dashboardwidget.h"
#include "qt/mk2x/sendconfirmdialog.h"
...
DashboardWidget::DashboardWidget(MK2XGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MK2XGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &MK2XGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &MK2XGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("MK2")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MK2, zMK2
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(215,144,32));
    set1->setColor(QColor(215,182,22));
