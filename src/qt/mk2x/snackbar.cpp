...
#include "qt/mk2x/forms/ui_snackbar.h"
#include "qt/mk2x/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MK2XGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MK2XGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
