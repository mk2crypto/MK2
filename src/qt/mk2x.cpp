...
#include "qt/mk2x/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("mk2x-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in mk2x.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in mk2x.qrc)
...
/** Class encapsulating MK2X Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (MK2XGUI)
...
{
    window = new MK2XGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MK2XGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MK2XGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(MK2XGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MK2XGUI::DEFAULT_WALLET);
        ...
        // MK2X: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MK2XGUI::handlePaymentRequest);
        connect(window, &MK2XGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. MK2X can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(mk2x_locale);
    Q_INIT_RESOURCE(mk2x);
    ...
    /// 6. Determine availability of data directory and parse mk2x.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("MK2X Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("MK2X Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // mk2x: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("MK2X Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
