...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MK2);
    ...
    if (settings.contains("nAnonymizeMk2xAmount") || reset)
        SoftSetArg("-anonymizemk2xamount", settings.value("nAnonymizeMk2xAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mk2x.cpp during shotdown to also remove the window positions
