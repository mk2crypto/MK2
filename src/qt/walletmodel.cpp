...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered mk2x address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal mk2x:URI (mk2x:XyZ...?message=example)
            {
...
bool WalletModel::createZmk2Spend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZmk2(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzMK2Reset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMK2Reset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMK2Reset.connect(boost::bind(NotifyzMK2Reset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMK2Reset.disconnect(boost::bind(NotifyzMK2Reset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid MK2X address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid MK2X address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from MK2X address");
...
{
    setMints.clear();
    setMints = pwalletMain->zmk2Tracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
