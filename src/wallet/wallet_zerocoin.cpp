    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZMK2Seed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZMK2Seed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zmk2seed to wallet";
    }
    //the use case for this is no password set seed, mint dzMK2,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZMK2Seed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZMK2Seed(hashSeed, vchSeed)) {
...
bool CWallet::CreateZMK2OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZMK2(denomination, coin, dMint);
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZMK2OutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zmk2 output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Mk2
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CBitcoinAddress* changeAddress)
{
    ...
    int nStatus = ZMK2_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZMK2_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZMK2_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZMK2_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zmk2Tracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zmk2Tracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZMK2_SPEND_OKAY);  // When we reach this point spending zMK2 was successful
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZMK2_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZMK2_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZMK2_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZMK2_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZMK2Module::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZMK2_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
    ...
    receipt.SetStatus(_("Spend Valid"), ZMK2_SPEND_OKAY); // Everything okay
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    int nStatus = ZMK2_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zMK2"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZMK2_TRX_CREATE;
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zMK2 public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zMK2 used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zmk2Tracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zMK2 mints to use in this spend
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zmk2Tracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zmk2Tracker->Get(hashSerial);
            meta.isUsed = true;
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to mk2x address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            unsigned int nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZMK2_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZMK2_SPEND_OKAY); // Everything okay
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zMK2 in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zmk2Tracker->GetMints(true);
    ...
    return zmk2Tracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zmk2Tracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zmk2Tracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzMK2Wallet* zwallet)
{
    zwalletMain = zwallet;
    zmk2Tracker = std::unique_ptr<CzMK2Tracker>(new CzMK2Tracker(this));
}

CzMK2Wallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
{
    return zmk2Tracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zmk2Tracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zmk2Tracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zmk2Tracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zmk2Tracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
{
    CMintMeta meta;
    if (!zmk2Tracker->GetMetaFromStakeHash(hashStake, meta))
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zmk2Tracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zmk2Tracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zmk2Tracker->UpdateState(meta);
    } else {
