...
// Copyright (c) 2016-2020 The MK2X developers
...
public:
    ...
    bool WriteZMK2Seed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZMK2Seed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZMK2Seed_deprecated(uint256& seed);
    bool EraseZMK2Seed();
    bool EraseZMK2Seed_deprecated();

    bool WriteZMK2Count(const uint32_t& nCount);
    bool ReadZMK2Count(uint32_t& nCount);
